#
# This file is auto-generated. DO NOT EDIT
# Generated by: autospec.py
#
Name     : linux-steam-integration
Version  : 0.7.2
Release  : 9
URL      : https://github.com/solus-project/linux-steam-integration/releases/download/v0.7.2/linux-steam-integration-0.7.2.tar.xz
Source0  : https://github.com/solus-project/linux-steam-integration/releases/download/v0.7.2/linux-steam-integration-0.7.2.tar.xz
Summary  : Common C library functions
Group    : Development/Tools
License  : LGPL-2.1
Requires: linux-steam-integration-bin
Requires: linux-steam-integration-license
Requires: linux-steam-integration-locales
BuildRequires : buildreq-meson
BuildRequires : pkgconfig(check)
BuildRequires : pkgconfig(gtk+-3.0)

%description
linux-steam-integration
-----------------------
Linux Steam* Integration is a helper system to make the Steam Client and Steam
games run better on Linux. In a nutshell, LSI automatically applies various workarounds
to get games working, and fixes long standing bugs in both games and the client.

%package bin
Summary: bin components for the linux-steam-integration package.
Group: Binaries
Requires: linux-steam-integration-license

%description bin
bin components for the linux-steam-integration package.


%package dev
Summary: dev components for the linux-steam-integration package.
Group: Development
Requires: linux-steam-integration-bin
Provides: linux-steam-integration-devel

%description dev
dev components for the linux-steam-integration package.


%package license
Summary: license components for the linux-steam-integration package.
Group: Default

%description license
license components for the linux-steam-integration package.


%package locales
Summary: locales components for the linux-steam-integration package.
Group: Default

%description locales
locales components for the linux-steam-integration package.


%prep
%setup -q -n linux-steam-integration-0.7.2

%build
export http_proxy=http://127.0.0.1:9/
export https_proxy=http://127.0.0.1:9/
export no_proxy=localhost,127.0.0.1,0.0.0.0
export LANG=C
export SOURCE_DATE_EPOCH=1532987024
CFLAGS="$CFLAGS" CXXFLAGS="$CXXFLAGS" LDFLAGS="$LDFLAGS" meson --prefix /usr --buildtype=plain -Dwith-steam-binary=/usr/bin/steam  builddir
ninja -v -C builddir

%install
mkdir -p %{buildroot}/usr/share/doc/linux-steam-integration
cp LICENSE %{buildroot}/usr/share/doc/linux-steam-integration/LICENSE
cp src/libnica/LICENSE.LGPL2.1 %{buildroot}/usr/share/doc/linux-steam-integration/src_libnica_LICENSE.LGPL2.1
DESTDIR=%{buildroot} ninja -C builddir install
%find_lang linux-steam-integration

%files
%defattr(-,root,root,-)

%files bin
%defattr(-,root,root,-)
/usr/bin/lsi-exec
/usr/bin/steam

%files dev
%defattr(-,root,root,-)
/usr/lib64/liblsi-intercept.so
/usr/lib64/liblsi-redirect.so

%files license
%defattr(-,root,root,-)
/usr/share/doc/linux-steam-integration/LICENSE
/usr/share/doc/linux-steam-integration/src_libnica_LICENSE.LGPL2.1

%files locales -f linux-steam-integration.lang
%defattr(-,root,root,-)

